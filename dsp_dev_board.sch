EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 4
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Device:R R9
U 1 1 5FFBB5A8
P 9400 1800
F 0 "R9" V 9193 1800 50  0000 C CNN
F 1 "47" V 9284 1800 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 9330 1800 50  0001 C CNN
F 3 "~" H 9400 1800 50  0001 C CNN
	1    9400 1800
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR014
U 1 1 5FFBC04C
P 9850 2300
F 0 "#PWR014" H 9850 2050 50  0001 C CNN
F 1 "GND" H 9855 2127 50  0000 C CNN
F 2 "" H 9850 2300 50  0001 C CNN
F 3 "" H 9850 2300 50  0001 C CNN
	1    9850 2300
	1    0    0    -1  
$EndComp
Wire Wire Line
	9850 2300 9850 2200
$Comp
L Device:D_Schottky D4
U 1 1 5FFC1783
P 9850 2050
F 0 "D4" V 9804 2129 50  0000 L CNN
F 1 "SS14" V 9895 2129 50  0000 L CNN
F 2 "Diode_SMD:D_SMA" H 9850 2050 50  0001 C CNN
F 3 "~" H 9850 2050 50  0001 C CNN
	1    9850 2050
	0    1    1    0   
$EndComp
Text GLabel 9850 1350 1    50   Input ~ 0
3.3V
Wire Wire Line
	9850 1350 9850 1400
$Comp
L Device:D_Schottky D3
U 1 1 5FFC2017
P 9850 1550
F 0 "D3" V 9804 1629 50  0000 L CNN
F 1 "SS14" V 9895 1629 50  0000 L CNN
F 2 "Diode_SMD:D_SMA" H 9850 1550 50  0001 C CNN
F 3 "~" H 9850 1550 50  0001 C CNN
	1    9850 1550
	0    1    1    0   
$EndComp
Wire Wire Line
	9850 1700 9850 1800
Wire Wire Line
	9550 1800 9850 1800
Connection ~ 9850 1800
Wire Wire Line
	9850 1800 9850 1900
$Comp
L Device:C C4
U 1 1 5FFCB053
P 8950 1800
F 0 "C4" V 8698 1800 50  0000 C CNN
F 1 "1u" V 8789 1800 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 8988 1650 50  0001 C CNN
F 3 "~" H 8950 1800 50  0001 C CNN
	1    8950 1800
	0    1    1    0   
$EndComp
Wire Wire Line
	9100 1800 9250 1800
Wire Wire Line
	8800 1800 8650 1800
$Comp
L Connector:Barrel_Jack J4
U 1 1 60004038
P 2050 1500
F 0 "J4" H 2050 1200 50  0000 C CNN
F 1 "9v_jack" H 2050 1300 50  0000 C CNN
F 2 "Connector_BarrelJack:BarrelJack_Horizontal" H 2100 1460 50  0001 C CNN
F 3 "~" H 2100 1460 50  0001 C CNN
	1    2050 1500
	1    0    0    1   
$EndComp
Text GLabel 3050 1400 2    50   Output ~ 0
9V
$Comp
L power:GND #PWR08
U 1 1 60005258
P 2500 1700
F 0 "#PWR08" H 2500 1450 50  0001 C CNN
F 1 "GND" H 2505 1527 50  0000 C CNN
F 2 "" H 2500 1700 50  0001 C CNN
F 3 "" H 2500 1700 50  0001 C CNN
	1    2500 1700
	1    0    0    -1  
$EndComp
Wire Wire Line
	2500 1700 2500 1600
Wire Wire Line
	2500 1600 2350 1600
$Comp
L power:GND #PWR012
U 1 1 6002C64C
P 5700 1400
F 0 "#PWR012" H 5700 1150 50  0001 C CNN
F 1 "GND" H 5705 1227 50  0000 C CNN
F 2 "" H 5700 1400 50  0001 C CNN
F 3 "" H 5700 1400 50  0001 C CNN
	1    5700 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5700 1400 5700 1350
Wire Wire Line
	5700 1350 5500 1350
Wire Wire Line
	5500 1350 5500 1600
Wire Wire Line
	5500 1600 5400 1600
Wire Wire Line
	5800 1800 5600 1800
$Sheet
S 5700 4500 750  1350
U 60038522
F0 "MCU" 50
F1 "mcu.sch" 50
F2 "BCLK" I R 6450 4950 50 
F3 "WCLK" I R 6450 5050 50 
F4 "I2S_SD" O R 6450 4850 50 
F5 "I2Sext_SD" I R 6450 4750 50 
F6 "SCLK" O R 6450 5200 50 
F7 "~CSB" O R 6450 5300 50 
F8 "SDOUT" O R 6450 5400 50 
F9 "I2S2_MCK" O R 6450 4650 50 
F10 "SDA" B R 6450 5550 50 
F11 "SCL" O R 6450 5650 50 
F12 "~CODEC_RST" O R 6450 5750 50 
$EndSheet
Text Label 10250 1800 0    50   ~ 0
audio_in
Wire Wire Line
	10250 1800 9850 1800
Text Label 6650 4450 2    50   ~ 0
audio_in
Wire Wire Line
	6950 4450 7100 4450
$Comp
L Device:R R7
U 1 1 60086C44
P 8250 2350
F 0 "R7" V 8043 2350 50  0000 C CNN
F 1 "10k" V 8134 2350 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8180 2350 50  0001 C CNN
F 3 "~" H 8250 2350 50  0001 C CNN
	1    8250 2350
	0    1    1    0   
$EndComp
Wire Wire Line
	8400 2350 8650 2350
Wire Wire Line
	8650 2350 8650 1800
Connection ~ 8650 1800
Wire Wire Line
	8650 1800 8600 1800
$Comp
L Device:C C2
U 1 1 6009770B
P 7200 1900
F 0 "C2" V 6948 1900 50  0000 C CNN
F 1 "1u" V 7039 1900 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 7238 1750 50  0001 C CNN
F 3 "~" H 7200 1900 50  0001 C CNN
	1    7200 1900
	0    1    1    0   
$EndComp
Wire Wire Line
	6850 1900 7050 1900
Text GLabel 2150 3500 1    50   Input ~ 0
9V
$Comp
L Device:R R1
U 1 1 600A09B1
P 2150 3800
F 0 "R1" H 2220 3846 50  0000 L CNN
F 1 "10k" H 2220 3755 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 2080 3800 50  0001 C CNN
F 3 "~" H 2150 3800 50  0001 C CNN
	1    2150 3800
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 600A106D
P 2150 4250
F 0 "R2" H 2220 4296 50  0000 L CNN
F 1 "10k" H 2220 4205 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 2080 4250 50  0001 C CNN
F 3 "~" H 2150 4250 50  0001 C CNN
	1    2150 4250
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 3950 2150 4050
$Comp
L power:GND #PWR04
U 1 1 600A2818
P 2150 4500
F 0 "#PWR04" H 2150 4250 50  0001 C CNN
F 1 "GND" H 2155 4327 50  0000 C CNN
F 2 "" H 2150 4500 50  0001 C CNN
F 3 "" H 2150 4500 50  0001 C CNN
	1    2150 4500
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 4500 2150 4400
Wire Wire Line
	2150 3500 2150 3650
Wire Wire Line
	2150 4050 2500 4050
Connection ~ 2150 4050
Wire Wire Line
	2150 4050 2150 4100
Text GLabel 2500 4050 2    50   Output ~ 0
4.5V
Text GLabel 7950 1700 0    50   Input ~ 0
4.5V
Wire Wire Line
	7350 1900 7400 1900
Wire Wire Line
	8000 1900 7850 1900
$Comp
L Device:R R5
U 1 1 60088EB7
P 7550 1900
F 0 "R5" V 7343 1900 50  0000 C CNN
F 1 "10k" V 7434 1900 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7480 1900 50  0001 C CNN
F 3 "~" H 7550 1900 50  0001 C CNN
	1    7550 1900
	0    1    1    0   
$EndComp
Wire Wire Line
	7850 1900 7850 2350
Wire Wire Line
	7850 2350 8100 2350
Connection ~ 7850 1900
Wire Wire Line
	7850 1900 7700 1900
Wire Wire Line
	7950 1700 8000 1700
Wire Wire Line
	9250 3050 9100 3050
$Comp
L Device:R R8
U 1 1 600B8877
P 8250 3600
F 0 "R8" V 8043 3600 50  0000 C CNN
F 1 "10k" V 8134 3600 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8180 3600 50  0001 C CNN
F 3 "~" H 8250 3600 50  0001 C CNN
	1    8250 3600
	0    1    1    0   
$EndComp
Wire Wire Line
	8400 3600 8650 3600
Wire Wire Line
	8650 3600 8650 3050
Wire Wire Line
	8650 3050 8600 3050
$Comp
L Device:C C3
U 1 1 600B8881
P 7200 3150
F 0 "C3" V 6948 3150 50  0000 C CNN
F 1 "1u" V 7039 3150 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 7238 3000 50  0001 C CNN
F 3 "~" H 7200 3150 50  0001 C CNN
	1    7200 3150
	0    1    1    0   
$EndComp
Text GLabel 7950 2950 0    50   Input ~ 0
4.5V
Wire Wire Line
	7350 3150 7400 3150
Wire Wire Line
	8000 3150 7850 3150
$Comp
L Device:R R6
U 1 1 600B888A
P 7550 3150
F 0 "R6" V 7343 3150 50  0000 C CNN
F 1 "10k" V 7434 3150 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7480 3150 50  0001 C CNN
F 3 "~" H 7550 3150 50  0001 C CNN
	1    7550 3150
	0    1    1    0   
$EndComp
Wire Wire Line
	7850 3150 7850 3600
Wire Wire Line
	7850 3600 8100 3600
Connection ~ 7850 3150
Wire Wire Line
	7850 3150 7700 3150
Wire Wire Line
	7950 2950 8000 2950
Text Label 6750 3150 2    50   ~ 0
audio_out
Wire Wire Line
	6750 3150 7050 3150
Text Label 8450 4450 0    50   ~ 0
audio_out
Wire Wire Line
	8150 4450 8000 4450
$Comp
L Amplifier_Operational:TL072 U1
U 3 1 600C8CB7
P 1600 4000
F 0 "U1" H 1558 4046 50  0000 L CNN
F 1 "TL072" H 1558 3955 50  0000 L CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 1600 4000 50  0001 C CNN
F 3 "http://www.ti.com/lit/ds/symlink/tl071.pdf" H 1600 4000 50  0001 C CNN
	3    1600 4000
	1    0    0    -1  
$EndComp
Text GLabel 1500 3550 1    50   Input ~ 0
9V
Wire Wire Line
	1500 3550 1500 3700
$Comp
L power:GND #PWR02
U 1 1 600D21C7
P 1500 4400
F 0 "#PWR02" H 1500 4150 50  0001 C CNN
F 1 "GND" H 1505 4227 50  0000 C CNN
F 2 "" H 1500 4400 50  0001 C CNN
F 3 "" H 1500 4400 50  0001 C CNN
	1    1500 4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	1500 4400 1500 4300
Wire Wire Line
	6450 4650 6800 4650
Wire Wire Line
	6450 4750 6800 4750
Wire Wire Line
	6450 4850 6800 4850
Wire Wire Line
	6450 4950 6800 4950
Wire Wire Line
	6450 5050 6800 5050
$Sheet
S 7100 4350 900  1150
U 60041F8F
F0 "Codec" 50
F1 "codec.sch" 50
F2 "audio_in" I L 7100 4450 50 
F3 "audio_out" O R 8000 4450 50 
F4 "MCLK" I L 7100 4650 50 
F5 "BCLK" O L 7100 4950 50 
F6 "WCLK" O L 7100 5050 50 
F7 "DAC" I L 7100 4850 50 
F8 "ADC" O L 7100 4750 50 
F9 "SCLK" I L 7100 5200 50 
F10 "~CSB" I L 7100 5300 50 
F11 "SDIN" I L 7100 5400 50 
$EndSheet
Wire Wire Line
	6450 5400 7100 5400
Wire Wire Line
	6450 5300 7100 5300
Wire Wire Line
	6450 5200 7100 5200
$Comp
L Device:C C5
U 1 1 600DC113
P 8950 3050
F 0 "C5" V 8698 3050 50  0000 C CNN
F 1 "1u" V 8789 3050 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 8988 2900 50  0001 C CNN
F 3 "~" H 8950 3050 50  0001 C CNN
	1    8950 3050
	0    1    1    0   
$EndComp
Wire Wire Line
	8800 3050 8650 3050
Connection ~ 8650 3050
NoConn ~ 5400 1700
$Comp
L power:GND #PWR013
U 1 1 601D9CA8
P 5700 2550
F 0 "#PWR013" H 5700 2300 50  0001 C CNN
F 1 "GND" H 5705 2377 50  0000 C CNN
F 2 "" H 5700 2550 50  0001 C CNN
F 3 "" H 5700 2550 50  0001 C CNN
	1    5700 2550
	1    0    0    -1  
$EndComp
Wire Wire Line
	5700 2550 5700 2500
Wire Wire Line
	5700 2500 5500 2500
Wire Wire Line
	5500 2500 5500 2750
Wire Wire Line
	5500 2750 5400 2750
NoConn ~ 5400 2850
$Comp
L Connector:AudioJack2_Switch J6
U 1 1 601D9CB6
P 5200 2950
F 0 "J6" H 5100 3400 50  0000 C CNN
F 1 "output_jack" H 5050 3300 50  0000 C CNN
F 2 "Connector_Audio:Jack_6.35mm_Neutrik_NMJ4HCD2_Horizontal" H 5200 3150 50  0001 C CNN
F 3 "~" H 5200 3150 50  0001 C CNN
	1    5200 2950
	1    0    0    -1  
$EndComp
Text Label 5800 1800 0    50   ~ 0
audio_jack_in
Text Label 5800 2950 0    50   ~ 0
audio_jack_out
Text Label 6850 1900 2    50   ~ 0
audio_jack_in
Text Label 9250 3050 0    50   ~ 0
audio_jack_out
Wire Wire Line
	2550 2400 2350 2400
Connection ~ 2550 2400
Wire Wire Line
	2550 2400 2550 2500
Wire Wire Line
	2550 2850 2550 2800
$Comp
L power:GND #PWR09
U 1 1 60006D77
P 2550 2850
F 0 "#PWR09" H 2550 2600 50  0001 C CNN
F 1 "GND" H 2555 2677 50  0000 C CNN
F 2 "" H 2550 2850 50  0001 C CNN
F 3 "" H 2550 2850 50  0001 C CNN
	1    2550 2850
	1    0    0    -1  
$EndComp
$Comp
L Device:C C1
U 1 1 60006D71
P 2550 2650
F 0 "C1" H 2665 2696 50  0000 L CNN
F 1 "10u" H 2665 2605 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 2588 2500 50  0001 C CNN
F 3 "~" H 2550 2650 50  0001 C CNN
	1    2550 2650
	1    0    0    -1  
$EndComp
Wire Wire Line
	1750 2400 1650 2400
Text GLabel 1450 2400 0    50   Input ~ 0
9V
Wire Wire Line
	2750 2400 2550 2400
Text GLabel 2750 2400 2    50   Output ~ 0
3.3V
Wire Wire Line
	2050 2750 2050 2700
$Comp
L power:GND #PWR03
U 1 1 5FFC34F5
P 2050 2750
F 0 "#PWR03" H 2050 2500 50  0001 C CNN
F 1 "GND" H 2055 2577 50  0000 C CNN
F 2 "" H 2050 2750 50  0001 C CNN
F 3 "" H 2050 2750 50  0001 C CNN
	1    2050 2750
	1    0    0    -1  
$EndComp
$Comp
L Regulator_Linear:AMS1117-3.3 U2
U 1 1 5FFC29B1
P 2050 2400
F 0 "U2" H 2050 2642 50  0000 C CNN
F 1 "AMS1117-3.3" H 2050 2551 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 2050 2600 50  0001 C CNN
F 3 "http://www.advanced-monolithic.com/pdf/ds1117.pdf" H 2150 2150 50  0001 C CNN
	1    2050 2400
	1    0    0    -1  
$EndComp
$Comp
L Connector:Conn_01x02_Male J1
U 1 1 601E5519
P 1800 5600
F 0 "J1" H 1550 5800 50  0000 C CNN
F 1 "aux_mcu_power" H 1550 5700 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x02_P2.54mm_Vertical" H 1800 5600 50  0001 C CNN
F 3 "~" H 1800 5600 50  0001 C CNN
	1    1800 5600
	1    0    0    -1  
$EndComp
Text GLabel 2150 5600 2    50   Input ~ 0
3.3V
$Comp
L power:GND #PWR05
U 1 1 601E6A36
P 2150 5750
F 0 "#PWR05" H 2150 5500 50  0001 C CNN
F 1 "GND" H 2155 5577 50  0000 C CNN
F 2 "" H 2150 5750 50  0001 C CNN
F 3 "" H 2150 5750 50  0001 C CNN
	1    2150 5750
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 5750 2150 5700
Wire Wire Line
	2150 5700 2000 5700
Wire Wire Line
	2000 5600 2150 5600
Text Label 2150 6200 0    50   ~ 0
audio_in
Wire Wire Line
	2150 6200 2000 6200
Text Label 2150 6850 0    50   ~ 0
audio_out
$Comp
L Connector:Conn_01x02_Male J2
U 1 1 6026815E
P 1800 6200
F 0 "J2" H 1550 6400 50  0000 C CNN
F 1 "audio_in" H 1550 6300 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x02_P2.54mm_Vertical" H 1800 6200 50  0001 C CNN
F 3 "~" H 1800 6200 50  0001 C CNN
	1    1800 6200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR06
U 1 1 6026A61A
P 2150 6350
F 0 "#PWR06" H 2150 6100 50  0001 C CNN
F 1 "GND" H 2155 6177 50  0000 C CNN
F 2 "" H 2150 6350 50  0001 C CNN
F 3 "" H 2150 6350 50  0001 C CNN
	1    2150 6350
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 6350 2150 6300
Wire Wire Line
	2150 6300 2000 6300
Wire Wire Line
	2150 6850 2000 6850
$Comp
L Connector:Conn_01x02_Male J3
U 1 1 60272181
P 1800 6850
F 0 "J3" H 1550 7050 50  0000 C CNN
F 1 "audio_out" H 1550 6950 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x02_P2.54mm_Vertical" H 1800 6850 50  0001 C CNN
F 3 "~" H 1800 6850 50  0001 C CNN
	1    1800 6850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR07
U 1 1 60272187
P 2150 7000
F 0 "#PWR07" H 2150 6750 50  0001 C CNN
F 1 "GND" H 2155 6827 50  0000 C CNN
F 2 "" H 2150 7000 50  0001 C CNN
F 3 "" H 2150 7000 50  0001 C CNN
	1    2150 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 7000 2150 6950
Wire Wire Line
	2150 6950 2000 6950
$Comp
L Device:D_Schottky D1
U 1 1 6020CE2E
P 1350 2750
F 0 "D1" H 1150 2900 50  0000 L CNN
F 1 "SS14" H 1300 2900 50  0000 L CNN
F 2 "Diode_SMD:D_SMA" H 1350 2750 50  0001 C CNN
F 3 "~" H 1350 2750 50  0001 C CNN
	1    1350 2750
	-1   0    0    1   
$EndComp
Text GLabel 1050 2750 0    50   Input ~ 0
VBUS
Wire Wire Line
	1500 2750 1650 2750
Wire Wire Line
	1650 2750 1650 2400
Connection ~ 1650 2400
Wire Wire Line
	1650 2400 1450 2400
Wire Wire Line
	1200 2750 1050 2750
$Comp
L Connector:AudioJack2_Switch J5
U 1 1 601CE4FA
P 5200 1800
F 0 "J5" H 5100 2250 50  0000 C CNN
F 1 "input_jack" H 5050 2150 50  0000 C CNN
F 2 "Connector_Audio:Jack_6.35mm_Neutrik_NMJ4HCD2_Horizontal" H 5200 2000 50  0001 C CNN
F 3 "~" H 5200 2000 50  0001 C CNN
	1    5200 1800
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 600E7C35
P 5600 1950
F 0 "R3" H 5670 1996 50  0000 L CNN
F 1 "1M" H 5670 1905 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 5530 1950 50  0001 C CNN
F 3 "~" H 5600 1950 50  0001 C CNN
	1    5600 1950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR010
U 1 1 600E8A0A
P 5600 2150
F 0 "#PWR010" H 5600 1900 50  0001 C CNN
F 1 "GND" H 5605 1977 50  0000 C CNN
F 2 "" H 5600 2150 50  0001 C CNN
F 3 "" H 5600 2150 50  0001 C CNN
	1    5600 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	5600 2150 5600 2100
$Comp
L Device:R R4
U 1 1 600EC6BA
P 5600 3100
F 0 "R4" H 5670 3146 50  0000 L CNN
F 1 "1M" H 5670 3055 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 5530 3100 50  0001 C CNN
F 3 "~" H 5600 3100 50  0001 C CNN
	1    5600 3100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR011
U 1 1 600EC6C0
P 5600 3300
F 0 "#PWR011" H 5600 3050 50  0001 C CNN
F 1 "GND" H 5605 3127 50  0000 C CNN
F 2 "" H 5600 3300 50  0001 C CNN
F 3 "" H 5600 3300 50  0001 C CNN
	1    5600 3300
	1    0    0    -1  
$EndComp
Wire Wire Line
	5600 3300 5600 3250
NoConn ~ 5400 1900
NoConn ~ 5400 3050
Wire Wire Line
	5800 2950 5600 2950
Connection ~ 5600 2950
Wire Wire Line
	5600 2950 5400 2950
Wire Wire Line
	5600 1800 5400 1800
Connection ~ 5600 1800
$Comp
L Device:D_Schottky D2
U 1 1 60286E4B
P 2700 1400
F 0 "D2" H 2550 1250 50  0000 L CNN
F 1 "SS14" H 2700 1250 50  0000 L CNN
F 2 "Diode_SMD:D_SMA" H 2700 1400 50  0001 C CNN
F 3 "~" H 2700 1400 50  0001 C CNN
	1    2700 1400
	-1   0    0    1   
$EndComp
Wire Wire Line
	3050 1400 2950 1400
Wire Wire Line
	2550 1400 2350 1400
$Comp
L power:PWR_FLAG #FLG01
U 1 1 606222E5
P 850 850
F 0 "#FLG01" H 850 925 50  0001 C CNN
F 1 "PWR_FLAG" H 850 1023 50  0000 C CNN
F 2 "" H 850 850 50  0001 C CNN
F 3 "~" H 850 850 50  0001 C CNN
	1    850  850 
	1    0    0    -1  
$EndComp
Text GLabel 850  1000 3    50   Output ~ 0
9V
Wire Wire Line
	850  1000 850  850 
$Comp
L power:PWR_FLAG #FLG02
U 1 1 60625618
P 1350 850
F 0 "#FLG02" H 1350 925 50  0001 C CNN
F 1 "PWR_FLAG" H 1350 1023 50  0000 C CNN
F 2 "" H 1350 850 50  0001 C CNN
F 3 "~" H 1350 850 50  0001 C CNN
	1    1350 850 
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR01
U 1 1 60626032
P 1350 950
F 0 "#PWR01" H 1350 700 50  0001 C CNN
F 1 "GND" H 1355 777 50  0000 C CNN
F 2 "" H 1350 950 50  0001 C CNN
F 3 "" H 1350 950 50  0001 C CNN
	1    1350 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	1350 950  1350 850 
$Comp
L Amplifier_Operational:TL072 U1
U 2 1 6045F95D
P 8300 1800
F 0 "U1" H 8300 2167 50  0000 C CNN
F 1 "TL072" H 8300 2076 50  0000 C CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 8300 1800 50  0001 C CNN
F 3 "http://www.ti.com/lit/ds/symlink/tl071.pdf" H 8300 1800 50  0001 C CNN
	2    8300 1800
	1    0    0    -1  
$EndComp
$Comp
L Amplifier_Operational:TL072 U1
U 1 1 6046444C
P 8300 3050
F 0 "U1" H 8300 3417 50  0000 C CNN
F 1 "TL072" H 8300 3326 50  0000 C CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 8300 3050 50  0001 C CNN
F 3 "http://www.ti.com/lit/ds/symlink/tl071.pdf" H 8300 3050 50  0001 C CNN
	1    8300 3050
	1    0    0    -1  
$EndComp
$Sheet
S 9300 4350 1150 1500
U 6080BC6B
F0 "TI Codec" 50
F1 "TI_Codec.sch" 50
F2 "audio_in" I L 9300 4450 50 
F3 "audio_out" O R 10450 4450 50 
F4 "SCL" I L 9300 5650 50 
F5 "SDA" B L 9300 5550 50 
F6 "WCLK" I L 9300 5050 50 
F7 "BCLK" I L 9300 4950 50 
F8 "MCLK" I L 9300 4650 50 
F9 "DIN" I L 9300 4850 50 
F10 "~CODEC_RST" I L 9300 5750 50 
F11 "DOUT" O L 9300 4750 50 
$EndSheet
Text Label 8950 4150 2    50   ~ 0
audio_in
Text Label 10800 4150 0    50   ~ 0
audio_out
Wire Wire Line
	6800 5050 6800 5000
Wire Wire Line
	6800 5000 9000 5000
Wire Wire Line
	9000 5000 9000 5050
Wire Wire Line
	9000 5050 9300 5050
Connection ~ 6800 5050
Wire Wire Line
	6800 5050 7100 5050
Wire Wire Line
	6800 4950 6800 4900
Wire Wire Line
	6800 4900 9000 4900
Wire Wire Line
	9000 4900 9000 4950
Wire Wire Line
	9000 4950 9300 4950
Connection ~ 6800 4950
Wire Wire Line
	6800 4950 7100 4950
Wire Wire Line
	6800 4850 6800 4800
Wire Wire Line
	6800 4800 9000 4800
Wire Wire Line
	9000 4800 9000 4850
Wire Wire Line
	9000 4850 9300 4850
Connection ~ 6800 4850
Wire Wire Line
	6800 4850 7100 4850
Wire Wire Line
	9300 4750 9000 4750
Wire Wire Line
	9000 4750 9000 4700
Wire Wire Line
	9000 4700 6800 4700
Wire Wire Line
	6800 4700 6800 4750
Connection ~ 6800 4750
Wire Wire Line
	6800 4750 7100 4750
Wire Wire Line
	6800 4650 6800 4600
Wire Wire Line
	6800 4600 9000 4600
Wire Wire Line
	9000 4600 9000 4650
Wire Wire Line
	9000 4650 9300 4650
Connection ~ 6800 4650
Wire Wire Line
	6800 4650 7100 4650
Wire Wire Line
	6450 5550 8550 5550
Wire Wire Line
	6450 5650 8850 5650
Wire Wire Line
	6450 5750 9300 5750
$Comp
L Regulator_Linear:AMS1117-1.8 U6
U 1 1 60865026
P 3750 2400
F 0 "U6" H 3750 2642 50  0000 C CNN
F 1 "AMS1117-1.8" H 3750 2551 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 3750 2600 50  0001 C CNN
F 3 "http://www.advanced-monolithic.com/pdf/ds1117.pdf" H 3850 2150 50  0001 C CNN
	1    3750 2400
	1    0    0    -1  
$EndComp
Text GLabel 3350 2400 0    50   Input ~ 0
9V
Wire Wire Line
	3350 2400 3450 2400
$Comp
L power:GND #PWR030
U 1 1 6086D2AE
P 3750 2750
F 0 "#PWR030" H 3750 2500 50  0001 C CNN
F 1 "GND" H 3755 2577 50  0000 C CNN
F 2 "" H 3750 2750 50  0001 C CNN
F 3 "" H 3750 2750 50  0001 C CNN
	1    3750 2750
	1    0    0    -1  
$EndComp
Wire Wire Line
	3750 2700 3750 2750
$Comp
L Device:C C36
U 1 1 6087562B
P 4200 2600
F 0 "C36" H 4315 2646 50  0000 L CNN
F 1 "10u" H 4315 2555 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 4238 2450 50  0001 C CNN
F 3 "~" H 4200 2600 50  0001 C CNN
	1    4200 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	4050 2400 4200 2400
Wire Wire Line
	4200 2400 4200 2450
$Comp
L power:GND #PWR031
U 1 1 6087C463
P 4200 2800
F 0 "#PWR031" H 4200 2550 50  0001 C CNN
F 1 "GND" H 4205 2627 50  0000 C CNN
F 2 "" H 4200 2800 50  0001 C CNN
F 3 "" H 4200 2800 50  0001 C CNN
	1    4200 2800
	1    0    0    -1  
$EndComp
Wire Wire Line
	4200 2750 4200 2800
Text GLabel 4350 2400 2    50   Output ~ 0
1.8V
Wire Wire Line
	4350 2400 4200 2400
Connection ~ 4200 2400
Wire Wire Line
	8550 5500 8550 5550
Connection ~ 8550 5550
Wire Wire Line
	8550 5550 9300 5550
$Comp
L Device:R R20
U 1 1 608A7F3C
P 8550 5350
F 0 "R20" H 8400 5300 50  0000 C CNN
F 1 "10k" H 8400 5400 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8480 5350 50  0001 C CNN
F 3 "~" H 8550 5350 50  0001 C CNN
	1    8550 5350
	-1   0    0    1   
$EndComp
$Comp
L Device:R R21
U 1 1 608A8BFD
P 8850 5350
F 0 "R21" H 8700 5300 50  0000 C CNN
F 1 "10k" H 8700 5400 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8780 5350 50  0001 C CNN
F 3 "~" H 8850 5350 50  0001 C CNN
	1    8850 5350
	-1   0    0    1   
$EndComp
Wire Wire Line
	8850 5500 8850 5650
Connection ~ 8850 5650
Wire Wire Line
	8850 5650 9300 5650
Wire Wire Line
	8850 5200 8850 5150
Wire Wire Line
	8850 5150 8550 5150
Wire Wire Line
	8550 5150 8550 5200
Text GLabel 8400 5100 0    50   Input ~ 0
3.3V
Wire Wire Line
	8400 5100 8550 5100
Wire Wire Line
	8550 5100 8550 5150
Connection ~ 8550 5150
$Comp
L Device:C C37
U 1 1 608FE6BA
P 2950 1650
F 0 "C37" H 3065 1696 50  0000 L CNN
F 1 "10u" H 3065 1605 50  0000 L CNN
F 2 "Capacitor_SMD:C_1206_3216Metric" H 2988 1500 50  0001 C CNN
F 3 "~" H 2950 1650 50  0001 C CNN
	1    2950 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	2950 1400 2950 1500
Connection ~ 2950 1400
Wire Wire Line
	2950 1400 2850 1400
$Comp
L power:GND #PWR0101
U 1 1 609053CE
P 2950 1850
F 0 "#PWR0101" H 2950 1600 50  0001 C CNN
F 1 "GND" H 2955 1677 50  0000 C CNN
F 2 "" H 2950 1850 50  0001 C CNN
F 3 "" H 2950 1850 50  0001 C CNN
	1    2950 1850
	1    0    0    -1  
$EndComp
Wire Wire Line
	2950 1800 2950 1850
$Comp
L Device:R R23
U 1 1 607E529D
P 6800 4450
F 0 "R23" V 6593 4450 50  0000 C CNN
F 1 "0 DNP" V 6684 4450 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6730 4450 50  0001 C CNN
F 3 "~" H 6800 4450 50  0001 C CNN
	1    6800 4450
	0    1    1    0   
$EndComp
$Comp
L Device:R R24
U 1 1 607E8DDA
P 8300 4450
F 0 "R24" V 8093 4450 50  0000 C CNN
F 1 "0 DNP" V 8184 4450 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8230 4450 50  0001 C CNN
F 3 "~" H 8300 4450 50  0001 C CNN
	1    8300 4450
	0    1    1    0   
$EndComp
Wire Wire Line
	9250 4450 9300 4450
$Comp
L Device:R R25
U 1 1 607EA958
P 9100 4150
F 0 "R25" V 8893 4150 50  0000 C CNN
F 1 "0 DNP" V 8984 4150 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 9030 4150 50  0001 C CNN
F 3 "~" H 9100 4150 50  0001 C CNN
	1    9100 4150
	0    1    1    0   
$EndComp
Wire Wire Line
	9250 4150 9250 4450
$Comp
L Device:R R26
U 1 1 607F9AAB
P 10650 4150
F 0 "R26" V 10443 4150 50  0000 C CNN
F 1 "0 DNP" V 10534 4150 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 10580 4150 50  0001 C CNN
F 3 "~" H 10650 4150 50  0001 C CNN
	1    10650 4150
	0    1    1    0   
$EndComp
Wire Wire Line
	10500 4150 10500 4450
Wire Wire Line
	10500 4450 10450 4450
$EndSCHEMATC
